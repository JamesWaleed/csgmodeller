...
//
// History:
//   2016/10/01: 1.0.0 Commented out section that loads first example (?)
//
...
$(document).ready(function() {
    ...
    $("#menu-right").height($(window).height());       // initial height

    $('#editFrame').height($(window).height());
    $(window).resize(function() {                // adjust the relevant divs
        $("#menu").height($(window).height());
        $("#menu-right").height($(window).height());
        $("#menuHandle").css({top: '45%'});
        $("#menuHandle-right").css({top: '45%'});
        $("#editFrame").height($(window).height());
      });
    setTimeout( function(){
      $('#menu').css('left','-280px');
      $('#menu-right').css('right','-280px');
...
$(document).ready(function() {
  ...
  /*
      ...
      docUrl = document.URL;
      params = {};
      docTitle = 'skateboard-rev3-final.jscad';
	...
	*/
      ...
      gProcessor.setJsCad(getSourceFromEditor(),"160926-skateboard-rev4.jscad"); //BT 10/1 replaced with current jscad file
    }
